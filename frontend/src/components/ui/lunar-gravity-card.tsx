...
const RealisticMoon = ({ onClick, colorMap }: { onClick?: () => void; colorMap: THREE.Texture }) => {
...
> {
  ...
  render() {
    if (this.state.hasError) {
      return this.props.fallback ?? <div className="absolute inset-0 bg-zinc-950" />;
    }
          ...
          <CanvasErrorBoundary>
            ...
            <group rotation={[Math.PI / 8, 0, 0]}>
              <Suspense fallback={null}>
                {moonMap && (
                  <>
                    <RealisticMoon colorMap={moonMap} onClick={() => { if (ringState === "hidden") setRingState("animating"); }} />
                    <AsteroidBelt colorMap={moonMap} ringState={ringState} massiveAsteroidsRef={massiveAsteroidsRef} />
                  </>
                )}
...
export { LunarGravityCard, LunarGravityCard as Component };
